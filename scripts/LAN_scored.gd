...
func _ready() -> void:
	...
	LANNetworkManager.votes_updated.connect(update_votes)
...
func start_game():
	decide_serve()
	play_wall_effect.rpc(LANNetworkManager.player1_serves)
	create_ball.rpc()
...
func _on_wall_effect_finished():
	current_fade_tween.kill()
	play_wall_effect(LANNetworkManager.player1_serves)
...
func _on_play_again_button_pressed() -> void:
	...
	if !LANNetworkManager.has_voted:
		LANNetworkManager.has_voted = true
		if multiplayer.is_server():
			LANNetworkManager.request_vote(1)
		else:
			LANNetworkManager.request_vote.rpc_id(1, 1)
	else:
		LANNetworkManager.has_voted = false
...
func waiting_for_rematch():
	...
	if LANNetworkManager.has_voted:
		...
		if multiplayer.is_server():
			if LANNetworkManager.players.has(LANNetworkManager.client_id):
				var client_id = LANNetworkManager.client_id
				
				set_colour(client_id )
				$Label.text = "You voted to play again, waiting for"
				
				if !LANNetworkManager.players[client_id ].name == "":
					label_player.text = LANNetworkManager.players[client_id].name
				else:
			...
			if !LANNetworkManager.players[1].name == "":
				label_player.text = LANNetworkManager.players[1].name
			else:
				label_player.text = "Player 1"
	else:
		if LANNetworkManager.votes == 1:
...
func set_label_position(id := 1):
	...
	if id == 1:
		name = LANNetworkManager.players[1].name
	else:
		name = LANNetworkManager.players[LANNetworkManager.client_id].name
...
func start_rematch():
	...
	if LANNetworkManager.players.size() == 2:
...
func set_colour(id := 1):
	if !LANNetworkManager.players.has(id):
		return
	
	var colour = LANNetworkManager.players[id].skin
