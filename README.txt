Instructions for Online Jam/Scored Multiplayer with Noray:

Before trying anything, on Windows, the first time each player opens the game they must click any Join button in the Online Scored or Online Jam menu and enter root password when
Windows asks for it, to prevent the Firewall from rejecting the game's connections.

Then one player clicks Host button and shares the OID code generated automatically when the game runs. Then the other player must paste it into the OID box (after deleting theirs) 
and click Join button. If the Handshake succeeds the Host can press Start button to start the game.
...
-If the Handshake fails it is recommended that both players (or at least the client) reset Noray (and share the new OID if host resets too) before trying again.
Player with better connection speed should be the host.


Instructions for LAN Jam/Scored Multiplayer:
...
Windows may ask you for root password in order to allow that connection, which is necessary otherwise its Firewall will reject it.
...
https://www.youtube.com/watch?v=YnfsyZJRsL8
